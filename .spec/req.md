...
#### 13.1.1 概要
管理画面で作成・編集した記事内容を基に、TOPページ（index.html）を静的HTMLとして生成する機能。

#### 13.1.2 更新方式
- **手動更新**: 管理画面から「TOPページ更新」ボタンを押すことで即座に生成
- **スケジュール更新**: 毎日定時（例：午前6時）に自動生成
- **記事公開時自動更新**: 記事を公開した際に自動的にTOPページを再生成（オプション）

#### 13.1.3 表示内容
- **最新情報セクション**
  - 最新5件の記事を表示
  - 各記事：タイトル、抜粋、カテゴリ、公開日時、「詳細へ」リンク
- **町会活動ハイライト**
  - 管理画面で「ピックアップ記事」として指定した記事を表示
  - アイキャッチ画像付き
  - 最大3件まで

#### 13.1.4 出力形式
- 完全な静的HTMLファイル（index.html）
- CSS/JSは別ファイルとして参照
- 画像URLは絶対パスで埋め込み
...
#### 13.2.1 概要
記事一覧とカレンダー表示を提供するページ（news.html または articles.html）。

#### 13.2.2 ページ構成
- **上部**: ページタイトル、検索ボックス、カテゴリフィルタ
- **中央**: 表示切替タブ（一覧表示 / カレンダー表示）
- **下部**: コンテンツエリア

#### 13.2.3 一覧表示セクション（静的生成）
- **更新方式**
  - 手動更新: 管理画面から「お知らせページ更新」ボタン
  - スケジュール更新: 毎日定時（例：午前6時）に自動生成
- **表示内容**
  - 記事一覧を静的HTMLとして生成
  - 各記事：タイトル、カテゴリ、公開日時、抜粋、「詳細へ」リンク
  - ページング（20件/ページ）は静的に生成
- **フィルタ・検索**
  - カテゴリフィルタ：静的HTMLで各カテゴリページを生成（news-event.html, news-notice.html 等）
  - 検索：Ajax APIを使用（リアルタイム検索）

#### 13.2.4 カレンダー表示セクション（Ajax動的表示）
- **更新方式**
  - リアルタイム表示：Supabase APIを使用して常に最新情報を取得
  - Ajaxで動的にカレンダーを描画
- **表示内容**
  - 月間カレンダー形式
  - イベント記事を日付に配置
...
- **操作機能**
  - 前月/次月ボタンで月を移動
  - 日付クリックで当日の記事を表示（モーダルまたはサイドパネル）
  - モバイルレスポンシブ対応
- **Ajax API連携**
  - GET /api/calendar?year=2025&month=11
  - レスポンス: { events: [{ date, title, category, url }, ...] }

#### 13.2.5 記事詳細ページ（静的生成）
- **ディレクトリ構造**
  - 記事詳細ページは `news/` フォルダ内に配置
  - テンプレート: `news/news_template.html`
  - 生成ファイル: `news/{slug}.html`
  - 相対パス設計: フォルダ内から親ディレクトリへは `../` でアクセス
- **更新方式**
  - 記事作成・編集時に個別の詳細ページを生成
  - ファイル名: news/{slug}.html
- **表示内容**
  - パンくずリスト（ホーム > お知らせ > 記事タイトル）
  - 記事本文（フルテキスト）
  - 公開日時、カテゴリ、タグ
  - アイキャッチ画像
  - イベント日時（イベント記事の場合）
  - 添付ファイルリスト
  - シェアボタン（LINE, X, Facebook）
  - 前後の記事へのナビゲーション
  - お知らせ一覧への戻るリンク
...
#### 13.3.1 HTML生成エンジン
- **テンプレートエンジン**（候補）
  - Jinja2（Python Lambda）
  - EJS（Node.js Lambda）
  - Handlebars（Node.js）
- **テンプレートファイル**
  - templates/index.html.j2 - TOPページテンプレート
  - templates/news.html.j2 - お知らせページテンプレート
  - templates/article-detail.html.j2 - 記事詳細テンプレート

#### 13.3.2 生成プロセス
1. データベース（Supabase）から記事データを取得
2. テンプレートエンジンでHTMLを生成
3. 生成したHTMLをS3にアップロード
4. CloudFront キャッシュを無効化

#### 13.3.3 スケジュール実行
- **AWS CloudWatch Events**
  - 毎日午前6時に Lambda 関数を起動
  - Lambda 関数: page-generator
  - 処理: TOPページとお知らせページを再生成

#### 13.3.4 手動更新
- **管理画面からのトリガー**
  - 「TOPページ更新」ボタン → POST /api/generate/index
  - 「お知らせページ更新」ボタン → POST /api/generate/news
  - 「全ページ更新」ボタン → POST /api/generate/all
- **API処理**
  - Lambda 関数を同期的に呼び出し
  - 生成完了後、成功メッセージを返却

### 13.4 Ajax API 仕様（カレンダー専用）
...
#### 13.5.1 静的ページ
- 読み込み時間: 2秒以内（初回アクセス）
- CloudFront キャッシュ活用: TTL 1時間
- 画像最適化: WebP形式、遅延ロード

#### 13.5.2 Ajax カレンダー
- API レスポンス時間: 500ms以内
- データ量制限: 1ヶ月あたり最大100イベント
- キャッシング: ブラウザキャッシュ（5分）
...
#### 13.7.2 Ajax部分のSEO
- カレンダー部分はクローラーに影響しない（補助機能）
- 主要コンテンツ（記事一覧）は静的HTMLで提供
...
- **静的生成** - データベースから取得したデータをHTMLファイルとして事前生成すること
- **Ajax** - 非同期通信を用いた動的コンテンツ表示
...
**文書作成日**: 2025年11月13日
**最終更新日**: 2025年12月07日
**ステータス**: 要件定義完了（TOPページ・お知らせページ要件追加、news フォルダ構造追加）
